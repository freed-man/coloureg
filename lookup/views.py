...
def index(request):
    ...
    if request.method == 'POST':
        ...
        if cached_payload:
            ...
            #
            # The caching rule further down already claims this is what
            # happens — "is_make_unsupported() is consulted live when a cached
            # row is served, so removing a make immediately resumes normal
            # lookups rather than serving a stale verdict". It was not true:
            # this branch read cached_payload['make_not_automated'], a bool
            ...
            # was flat False. But the gate is now decided live above, so a
            # third kind exists: an entry cached as gated whose verdict has
            # since changed (a make removed from the list). That row has no
            # paint code and no longer has a reason not to look for one, so
            # recovery must run for it.
            #
            # Served from cache WITH recovery on, rather than falling through
            # to a fresh lookup: falling through pushes the request down to the
            # per-registration window, which can REFUSE it — turning away a
            # paid, zero-cost cached repeat, the exact drift the placement
